...
from profiles.serializers import ProfileSerializer, PopularFollowerSerializer
...
class ProfileTests(TestCase):
    ...
    def _create_user(self, profile_name, email=None, password="StrongPassword123!"):
        ...
        user = User.objects.create_user(profile_name=profile_name, email=email, password=password)
        user.is_active = True
        user.save()
        return user

    def test_public_can_view_profile(self):
        """Public users can view profiles."""
    ...
    def test_unauthorized_user_cannot_update_profile(self):
        """Unauthorized users cannot update other users' profiles."""
    ...
    def test_follower_count_updates_on_follow(self):
        """Follower count updates correctly when a user is followed."""
    ...
    def test_profile_list_is_paginated(self):
        """Profile list is paginated."""
        for i in range(15):
            self._create_user(f'user{i}')
        response = self.client.get(self.profile_list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 10)
        self.assertIn('next', response.data)  # Check if pagination has 'next' page

    def test_profile_update_with_invalid_data(self):
        """Invalid profile update returns error."""
        self.client.force_authenticate(user=self.user)
        data = {"bio": "a" * 501}  # Bio exceeds max length of 500
        response = self.client.patch(self.update_profile_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_profile_list_ordering_by_popularity(self):
        """Profiles are ordered by popularity score."""
        self.user1.profile.popularity_score = 50
        self.user1.profile.save()
        self.user2.profile.popularity_score = 100
        self.user2.profile.save()
        response = self.client.get(self.profile_list_url)
        self.assertEqual(response.data['results'][0]['profile_name'], self.user2.profile.user.profile_name)
        self.assertEqual(response.data['results'][1]['profile_name'], self.user1.profile.user.profile_name)

    def test_profile_serializer_hides_email_for_non_owner(self):
        """Profile serializer hides the email for non-owners."""
        self.client.force_authenticate(user=self.user2)
    ...
    def test_update_all_popularity_scores_task(self, mock_task):
        """Popularity scores update task runs successfully."""
